...
static int pre_dump_one_task(struct pstree_item *item, InventoryEntry *parent_ie)
{
	...
	/*
	 * Send pidfd of task over pidfd_out_fd unix socket if option
	 * is set. This pidfd will be used in the next pre-dump/dump
	 * iteration in detect_pid_reuse().
	 */
	if (opts.pidfd_out_fd != -1) {
