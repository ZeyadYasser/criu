...
static int nftables_lock_connection_raw(int family, u32 *src_addr, u16 src_port,
						u32 *dst_addr, u16 dst_port)
{
#if defined(CONFIG_HAS_NFTABLES_LIB_API_0) || defined(CONFIG_HAS_NFTABLES_LIB_API_1)
	struct nft_ctx *nft;
	...
	snprintf(buf, sizeof(buf), NFTABLES_CONN_CMD,
			family == AF_INET ? '4' : '6',
			dip, (int)dst_port, sip, (int)src_port);
...
int nftables_lock_connection(struct inet_sk_desc *sk)
{
	...
	ret = nftables_lock_connection_raw(sk->sd.family,
			sk->src_addr, sk->src_port,
			sk->dst_addr, sk->dst_port);
	if (ret)
		return -1;
